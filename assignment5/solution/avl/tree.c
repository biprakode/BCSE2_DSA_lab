...
void descending_recursive(node *current , void **nodes , int *i);
void **descending(tree *tree_obj);
...
void **descending(tree *tree_obj) {
    ...
    int i = 0;
    descending_recursive(tree_obj->root, nodes, &i);
    return nodes;
}

void descending_recursive(node *current , void **nodes , int *i) {
    if (!current) return;
    descending_recursive(current->right, nodes, i);
    nodes[*i] = current->data;
    (*i)++;
    descending_recursive(current->left, nodes, i);
}
